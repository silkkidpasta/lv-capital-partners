...
export default function DashboardPage() {
  ...
  let trackEvent: ((event: string, properties?: Record<string, unknown>) => void) | null = null;
  try {
  ...
  // Track page view
  useEffect(() => {
    if (user && trackEvent && dashboardData) {
      trackEvent.portfolioViewed(
        dashboardData.portfolio.totalValue,
        dashboardData.portfolio.activeInvestments,
        user.id,
      );
    }
  }, [user, trackEvent, dashboardData]);

  const handleTabChange = (tab: string) => {
    setSelectedTab(tab);
    if (user && trackEvent) {
      trackEvent.contentEngagement("dashboard_tab", tab, "tab_view", user.id);
    }
  };
